...
-- NÃO carrega. Depois de rodá-lo, importe os dados por cima (CSVs pelo Table
-- Editor, ou `pg_restore --data-only`).
--
-- IDEMPOTENTE (desde 31/07/2026): pode ser rodado contra banco VAZIO ou contra
-- banco JÁ POVOADO, quantas vezes for preciso. Até esta data ele só funcionava
-- contra banco vazio — os 18 `CREATE TABLE` eram crus — e mesmo assim o runbook
-- do `backup.md` (caminho A) mandava rodá-lo DEPOIS de um `pg_restore` completo,
-- que já tinha criado tudo. O script abortava no primeiro "already exists" e,
-- como o SQL Editor roda o arquivo como um lote só, NADA das seções 6-8 era
-- aplicado: o banco restaurado ficava sem os GRANTs endurecidos e sem os
-- default privileges fechados — exatamente o buraco que a seção 7 existe para
-- tapar, aberto em silêncio e no pior momento possível. Achado da auditoria
-- cruzada de 31/07/2026 (item 1).
--
-- ⚠️ IDEMPOTENTE NÃO É RECONCILIADOR: `CREATE TABLE IF NOT EXISTS` PULA uma
-- tabela que já exista com formato DIFERENTE, sem avisar. Este arquivo garante
-- que a postura de segurança (seções 6-8) seja aplicada; ele não conserta uma
-- estrutura divergente. Para conferir estrutura, use o passo 8 do backup.md.
--
-- COMO USAR (em caso de perda total do banco):
--   1. Crie um projeto Supabase novo (ou zere o atual, com cuidado).
--   2. Restaure estrutura + dados com `pg_restore` (caminho A do backup.md);
--      OU, sem dump, siga direto para o passo 3 e importe CSVs depois.
--   3. SQL Editor → cole este arquivo inteiro → Run. Num banco recém-restaurado
--      isto é o que reaplica RLS, policies, GRANTs e default privileges — o
--      `pg_restore` do runbook usa `--no-privileges`, que DESCARTA os GRANTs.
--   4. Importe os dados, se ainda não vieram do dump.
--   5. Confira o Realtime (seção no fim) e o vercel.json/CSP — nada aqui
...
CREATE TABLE IF NOT EXISTS public.tabela_vista_teste (
  id                integer,
...
CREATE TABLE IF NOT EXISTS public.codempresa_teste (
...
CREATE TABLE IF NOT EXISTS public.tarifa_atual_teste (
  codempresa           character varying,
...
CREATE TABLE IF NOT EXISTS public.qh_teste (
  id                 integer PRIMARY KEY,
  codlinha           character varying,
  codempresa         character varying,
...
-- BY DEFAULT permite restaurar os row_id trazidos pelos CSVs; ALWAYS quebra o passo 4.
-- A proteção perdida é só teórica: o portal é read-only e o ETL omite row_id, portanto no uso
-- normal BY DEFAULT age como ALWAYS. Isso foi medido no restore de 28/07/2026, não deduzido.
CREATE TABLE IF NOT EXISTS public.qh_intervalo_teste (
  id            integer,
  codlinha      character varying,
  dia_semana    text,
...
CREATE TABLE IF NOT EXISTS public.qh_predeterminado_teste (
  id            integer,
  codlinha      character varying,
  dia_semana    text,
  cod_origem    character varying,
...
CREATE TABLE IF NOT EXISTS public.itinerario_teste (
  id                    numeric,
  codlinha              character varying,
...
CREATE TABLE IF NOT EXISTS public.evento_teste (
  id                integer PRIMARY KEY,
  codempresa        text,
  codlinha          text,
  data_registro     date,
...
CREATE TABLE IF NOT EXISTS public.evento_empresa_teste (
  id              integer,
  evento_empresa  text,
  row_id          bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY
);

CREATE TABLE IF NOT EXISTS public.evento_linha_teste (
  id            integer,
  evento_linha  text,
  row_id        bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY
);

CREATE TABLE IF NOT EXISTS public.portaria_teste (
  id                  integer PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS public.municipio_teste (
...
CREATE TABLE IF NOT EXISTS public.localidades_teste (
  localidade        text,
  ordem_importacao  integer PRIMARY KEY
);

CREATE TABLE IF NOT EXISTS public.origem_teste (
...
CREATE TABLE IF NOT EXISTS public.evento_dados (
  id                integer PRIMARY KEY,
  codempresa        text,
  codlinha          text,
  data_registro     date,
...
CREATE TABLE IF NOT EXISTS public.evento_textos (
  id                integer PRIMARY KEY,
  numero_processo   text,
  descricao         text,
  observacao        text
...
-- pelo portal; RLS fica ligado SEM policy de propósito (ver docs/schema.md).
CREATE TABLE IF NOT EXISTS public.portaria_data (
  id                  integer PRIMARY KEY,
  numero_portaria     character varying,
  data_portaria       date,
  data_publicacao     date,
...
CREATE TABLE IF NOT EXISTS public.portaria_texto_teste (
  id                integer PRIMARY KEY,
  tipo_legislacao   text,
  assunto           text,
  conteudo          text
...
-- ============================================================
-- DROP antes do ADD porque `ADD CONSTRAINT` não aceita IF NOT EXISTS: sem isto o script só
-- rodava contra banco vazio (ver o cabeçalho sobre idempotência). Recriar a FK REVALIDA os
-- dados existentes — num banco já restaurado isso é uma varredura, e se ela FALHAR o restore
-- estava inconsistente e você quer saber disso.
ALTER TABLE public.tarifa_atual_teste DROP CONSTRAINT IF EXISTS fk_tarifa_linha;
...
-- ============================================================
CREATE INDEX IF NOT EXISTS idx_codempresa_codempresa ON public.codempresa_teste USING btree (codempresa);

CREATE INDEX IF NOT EXISTS idx_evento_codempresa ON public.evento_teste USING btree (codempresa);
CREATE INDEX IF NOT EXISTS idx_evento_codlinha ON public.evento_teste USING btree (codlinha);
CREATE INDEX IF NOT EXISTS trgm_evento_descricao ON public.evento_teste USING gin (descricao gin_trgm_ops);
CREATE INDEX IF NOT EXISTS trgm_evento_observacao ON public.evento_teste USING gin (observacao gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_itinerario_cod_municipio_origem ON public.itinerario_teste USING btree (cod_municipio_origem);
CREATE INDEX IF NOT EXISTS idx_itinerario_codlinha ON public.itinerario_teste USING btree (codlinha);
CREATE INDEX IF NOT EXISTS trgm_itinerario_logradouro ON public.itinerario_teste USING gin (nome_logradouro gin_trgm_ops);
...
CREATE INDEX IF NOT EXISTS idx_portaria_data ON public.portaria_teste USING btree (data_portaria);
CREATE INDEX IF NOT EXISTS trgm_portaria_assunto ON public.portaria_teste USING gin (assunto gin_trgm_ops);
CREATE INDEX IF NOT EXISTS trgm_portaria_conteudo ON public.portaria_teste USING gin (conteudo gin_trgm_ops);
CREATE INDEX IF NOT EXISTS trgm_portaria_numero ON public.portaria_teste USING gin (numero_portaria gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_qh_intervalo_cod_origem ON public.qh_intervalo_teste USING btree (cod_origem);
CREATE INDEX IF NOT EXISTS idx_qh_intervalo_codlinha ON public.qh_intervalo_teste USING btree (codlinha);

CREATE INDEX IF NOT EXISTS idx_qh_predeterminado_codlinha ON public.qh_predeterminado_teste USING btree (codlinha);

CREATE INDEX IF NOT EXISTS idx_qh_codlinha ON public.qh_teste USING btree (codlinha);

CREATE INDEX IF NOT EXISTS idx_tabela_vista_codempresa ON public.tabela_vista_teste USING btree (codempresa);
CREATE INDEX IF NOT EXISTS idx_tabela_vista_codlinha ON public.tabela_vista_teste USING btree (codlinha);
CREATE INDEX IF NOT EXISTS trgm_tabela_vista_nome ON public.tabela_vista_teste USING gin (nome_ligacao gin_trgm_ops);
CREATE INDEX IF NOT EXISTS trgm_tabela_vista_nome_cresc ON public.tabela_vista_teste USING gin (nome_lig_cresc gin_trgm_ops);
CREATE INDEX IF NOT EXISTS trgm_tabela_vista_via ON public.tabela_vista_teste USING gin (via gin_trgm_ops);

CREATE INDEX IF NOT EXISTS idx_tarifa_codempresa ON public.tarifa_atual_teste USING btree (codempresa);
CREATE INDEX IF NOT EXISTS idx_tarifa_codempresa_codlinha ON public.tarifa_atual_teste USING btree (codempresa, codlinha);
CREATE INDEX IF NOT EXISTS idx_tarifa_codlinha ON public.tarifa_atual_teste USING btree (codlinha);
CREATE INDEX IF NOT EXISTS trgm_tarifa_nome ON public.tarifa_atual_teste USING gin (nome_ligacao gin_trgm_ops);
...
CREATE INDEX IF NOT EXISTS trgm_itin_logr_tipo_nome_norm ON public.itinerario_teste
...
DROP TRIGGER IF EXISTS trg_vigor_auto ON public.portaria_teste;
...
-- ============================================================
-- Laço com guarda em vez de 14 `ALTER PUBLICATION ... ADD TABLE` soltos: ADD TABLE erra com
-- "already member of publication" se a tabela já estiver lá, e um pg_restore traz a publicação
-- junto. A lista continua explícita — é ela que precisa bater com RT_TABLES no app.js.
DO $$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY[
    'codempresa_teste', 'evento_empresa_teste', 'evento_linha_teste', 'evento_teste',
    'localidades_teste', 'municipio_teste', 'origem_teste', 'portaria_teste',
    'tabela_vista_teste', 'itinerario_teste', 'qh_teste', 'qh_intervalo_teste',
    'qh_predeterminado_teste', 'tarifa_atual_teste'
  ] LOOP
    IF NOT EXISTS (

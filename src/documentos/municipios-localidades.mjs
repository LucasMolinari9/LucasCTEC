/* Família C4 — Municípios · Localidades.

   Os quatro loaders de consulta e todos os helpers exclusivos destas famílias recebem o contexto
   explícito `ctx = { view, gen, pane, host, line }`. O módulo não lê estado mutável do IIFE.
   Tabelas, paginação e seleção de linhas vêm de `ui/listas.mjs`; nenhum markup foi promovido a
   `ui/blocos.mjs`, pois os blocos abaixo são exclusivos desta família.
...
export async function ligacoesPorLogradouro(ctx){
  ...
  montarPainelBusca(ctx, { title:'Ligações por Logradouro', placeholder:'Nome da via / logradouro', selectOpts:[['','Todos os municípios'],...munOpts], onRun: async(term, { view, gen, host }, ibgeCod)=>{
...
export async function municipioRegiao(ctx){
  const ibge = await getIbge();
  ...
  montarPainelBusca(ctx, { title:'Município e Região', placeholder:'Nome do município (ou escolha uma região)', selectOpts:[['','Todas as regiões'],...regioes.map(r=>[r,r])], onRun: async(term, rctx, region)=>{
    ...
    if(region){
      ...
      scope.value = rctx.view?._regScope || 'origem';
      ...
      async function paint(){
        ...
        if(rctx.view) rctx.view._regScope = modo;
...
function openLinhasPorIbge(codibge, nome){
  abrirView({ title:'Linhas no Município', tables:['itinerario_teste','tabela_vista_teste','codempresa_teste'], loader: async(ctx)=>{
    ...
    scope.value = view?._munScope || 'todas';
    ...
    async function paint(){
      ...
      if(view) view._munScope = scope.value;
...
export async function ligacoesPorTerminal(ctx){
  ...
  montarPainelBusca(ctx, { title:'Ligações por Terminais', placeholder:'Nome do terminal / origem', selectOpts:[['','Todos os municípios'],...munOpts], suggest, onRun: async(term, rctx, ibgeCod)=>{
...
export function invalidarLocalidades(){ _localidadesList = null; }
/* --- DOC · Localidades -------------------------------------------- */
...
export async function localidades(ctx){
